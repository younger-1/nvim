-- <https://github.com/nyxkrage/NightVim/raw/main/sysinit.lua>
  ...
  if colorbuddy then
    ...
    if not ok then
      vim.log.err(
        '[COLOR] Colorscheme `'
          .. theme
          .. '` is not a valid colorschem, make sure you ran :PackerSync and restarted Neovim'
      )
    end
  else
...
local function init(autocmd_opts)
  ...
  if filename == 'init.lua' then
    ...
    if not ok then
      night.log.warn(
        'Error detected while proccessing'
